...
\citep{nostalgebraist2020}. What is new is the systematic application to \textit{reading} rather than steering,
...
The variants diverge only in \textit{magnitude}: residual-stream norms grow across
...
expected. The interesting result is the \textit{layer profile} --- at which layer does
...
($z = 223$--$4249$). This establishes that the \textit{direction} of $\dh$ matters, not
...
specific contrastive \textit{direction} from multiple pairs and test its causal effect
...
\textbf{Activation patching at multiple layers and positions:}
...
\textbf{Trajectory (some pole $+$, all pole $-$):}
...
All) onto the some$\leftrightarrow$all axis at L28:
...
name-sets (0.16--0.37), keying on the \textit{position} regardless of which name fills
...
prompt differs from the clean one in a \textit{single} input token --- the second
...
The real pole reads \textit{factual associations} (Tesla $\rightarrow$ alternating current,
Curie $\rightarrow$ radioactivity). The fictional pole reads \textit{name fragments} (Vog, von,
...
norm spikes at the Saturday$\rightarrow$Sunday and Sunday$\rightarrow$Monday transitions:
...
Paris token logit is \textit{higher} on the counterfactual side ($+9.6$) than on the
...
Projecting different negation types onto the contrastive \texttt{not} direction:

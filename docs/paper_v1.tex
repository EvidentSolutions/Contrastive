...
specifically. Section 3.4 tests whether the token-readable component of $\dhh$
...
method reads --- the token-space content of $\dhh$ --- correspond to structure the
...
from $\dhh$; neuron 7828 internally detects and gates on the same feature.
...
the contrastive method reads from $\dhh$.}
\label{tab:neurons}
\end{table}
